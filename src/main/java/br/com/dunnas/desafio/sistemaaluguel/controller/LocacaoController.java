...
public class LocacaoController {
    ...
    @GetMapping("/locacao/{id}/conteudo")
    public String exibirConteudoRestrito(@PathVariable("id") Integer locacaoId, 
                                         HttpSession session, 
                                         Model model) {
        ...
        try {
            ...
            // Garante que o utilizador logado é o dono desta locação.
            if (!locacao.getCliente().getId().equals(usuarioLogado.getId())) {
                // Se não for, nega o acesso.
                return "redirect:/meus-alugueis"; 
            }

            // Se tudo estiver OK, envia o objeto de locação para a página
            model.addAttribute("locacao", locacao);
            return "conteudo-restrito"; // Nome do novo ficheiro JSP
